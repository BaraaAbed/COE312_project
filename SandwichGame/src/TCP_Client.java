...
// -------------------------------- new reader -------------------------------
    ...
    public boolean putIngredient(double duration, String ingName) {
        ...
        boolean LR = false;
        boolean shake = false;
        System.out.println("Pick up the " + ingName + " to place it on the sandwich");
        while ((System.currentTimeMillis() - initTime) < duration*1000) {
            if (!LL && !LR && !shake) {
                synchronized (lockOrientation) {
                    if (orientation == 3) {
                ...
                while ((System.currentTimeMillis() - bla) < 5000) {
                    synchronized (lockOrientation) {
                        if (orientation == 4) {
                            LR = true;
                            bla = 0;
                ...
                if (peakAccAboveThreshold('X', 3, 5.0)) {
                    ...
                    audio.playSound("failedIngredientPlacement");
